...
export class Context extends EventEmitter {
   ...
   * const debug = debugFactory('tib:context:application');
  ...
  constructor(_parent?: Context | string, name?: string) {
    ...
    // See https://github.com/strongloop/tib-next/issues/4363
   ...
   *   return 'tib:context:application';
   * }
   * ```
   */
  protected getDebugNamespace() {
    if (this.constructor === Context) return 'tib:context';
    const name = this.constructor.name.toLowerCase();
    return `tib:context:${name}`;
  }
  ...
  close() {
    ...
    // https://github.com/strongloop/tib-next/issues/5943
    this._debug.destroy();
  }
